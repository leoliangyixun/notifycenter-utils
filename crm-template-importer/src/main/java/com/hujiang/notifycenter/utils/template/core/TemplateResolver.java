...
public abstract class TemplateResolver {
    ...
    public abstract void execute(List<Integer> templateIds, List<TemplateListDto> optionalTemplates, List<TemplateListDto> requiredTemplates);
        ...
        for (CrmTemplateGroupDto e : list) {
            if (CollectionUtils.isNotEmpty(e.getContents())) {
                ...
                if (isValid) {
                    ...
                    this.execute(templateIds, optionalTemplates, requiredTemplates);
                    ...
                    group.setTitle(
                        "CRM导入[" + e.getTemplateId() + ":" + e.getTemplate().getId() + "]-必发[" + StringUtils
